...
import type { JSX, CSSProperties, MouseEvent as ReactMouseEvent } from "react";
...
export default function ColorPicker({
                                      color,
                                      onChange,
                                    }: Readonly<IColorPickerProps>): JSX.Element {
  ...
  const onMoveSaturation = ({ x, y }: IPosition) => {
    const newHsv = {
      ...selfColor.hsv,
      s: (x / WIDTH) * 100,
      v: 100 - (y / HEIGHT) * 100,
...
function MoveWrapper({ className, style, onChange, children }: IMoveWrapperProps) {
  const divRef = useRef<HTMLDivElement>(null);
  const draggedRef = useRef(false);

  const move = (e: ReactMouseEvent | MouseEvent): void => {
    if (divRef.current) {
